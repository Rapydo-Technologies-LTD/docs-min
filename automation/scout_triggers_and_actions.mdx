...
```sql
Pattern: %DROP TABLE%
Result: Blocks any query attempting to drop tables
```

**Performance - Block Queries Without LIMIT:**
```sql
Pattern (Not Like): %LIMIT%
Result: Blocks SELECT queries missing pagination
```

**Compliance - Monitor Sensitive Data Access:**
```sql
Pattern: %SELECT * FROM salary%
Result: Alerts when salary table is queried
```
...
Result: Team is alerted to potentially dangerous query,
        but query is not blocked (allows investigation)
```
...
Action: Kill query + Notification
```
...
### Example 4: Block Dangerous Patterns

**Scenario:** Prevent destructive operations from non-admin users
...
**Result:** DROP TABLE attempts from non-admins are blocked and reported
