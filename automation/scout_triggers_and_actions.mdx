...
![Scout Rules List](/images/automation-screenshot.png)
...
![Create Scout Rule](/images/automation-create-scout-rule.png)
...
|-------|----------|-------------|
| **Value** | Yes | Maximum number of idle connections to kill per cycle |
...
**⚠️ Required:** A notification destination (email or webhook) must be configured for Query Analysis (RCA) to work. The analysis report cannot be delivered without a valid notification target.
...
![Scout Rule Notification](/images/automation-scout-rule-notification.png)

Sends an alert via email or webhook without taking any database action. Queries continue running unaffected.

**How to configure it in the UI:**
Select **No action** as the action type, then enable notifications and enter your email or webhook destination. This combination monitors and alerts without intervening.
...
![Execution Logs](/images/automation-execution-logs.png)

The Execution Logs panel shows a history of all rule activations — when each rule fired, which query triggered it, and what action was taken. Use this to verify rules are working correctly and to review past interventions.

---

## Best Practices

✅ **Start with No action + notification** before enabling kill actions—understand query patterns first
