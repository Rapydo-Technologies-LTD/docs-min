...
- Monitor CPU, memory, connections, IOPS, locks, deadlocks
...
- **Host**: Filter by client connection source
...
Combine multiple metrics with AND/OR logic for sophisticated monitoring.
...
**Notification Only**
- Sends alert via email or webhook without taking any database action
...
- **Use when:** 
  - Testing rules before enabling actions
  - Gathering data to understand query patterns
  - Building baseline metrics before deciding on thresholds
- **Example:** Monitor queries from `analytics_user` for one week to understand duration patterns before setting kill thresholds
...
Action: Webhook to #critical-alerts
```
...
```
Metric 1: Query Avg Time > 5 seconds
AND
Metric 2: Locks > 50

Action: Webhook to #database-ops
```
